...
- **2026-05-26 (v1.7)** — **Phase 2 substantially complete.** Tasks 2.5–2.15 executed: NATS bus wiring + audit envelope + middleware (2.5), Vault transit Ed25519 signing for envelopes (2.6), audit-appender in-process MVP (2.8), audit-projector immudb→Postgres (2.9), audit viewer Next.js + FastAPI JSON API (2.10), `oc` CLI audit subcommands (2.11), GitHub App `oc-attestation-bot` + Vault bootstrap (2.12), daily Merkle attestation publisher with first commit to `rky-2023/openclaw-attestations` (2.13), `oc audit replay/verify --fast` Postgres projection path (2.14), phase-2 smoke test + ops runbook (2.15). Deferred to Phase 2 completion: FastAPI audit middleware wiring (task 2.14 formal) + T8–T13 smoke tests that require a live audit pipeline. Phase 3 (event ingestion) runbook ready.

...
**and the LUKS data volumes do not auto-unlock**, so `oc-vault`/`oc-immudb`
crash-loop against their empty bind mountpoints (`/mnt/openclaw/{vault,immudb}`
unmounted). Recovery is in `RUNBOOK.md → "Unseal Vault after a host reboot"`:
`sudo bash infra/scripts/00-create-luks-volumes.sh` (re-unlock+mount) →
`docker compose … up -d --force-recreate vault immudb` →
`bash infra/scripts/03-unseal-vault.sh`. **Never chown + start an empty volume
dir** — it inits a fresh store and loses the audit chain. Before the smoke,
confirm `docker ps` shows `oc-vault`/`oc-immudb`/`oc-nats`/`oc-opa` healthy and
`docker exec oc-vault vault status` → `Sealed: false`.
